...
class UtilFunc
{
     ...
     */
    static struct drand48_data rand_buffers_[g_max_thread_num];
...
public:
    ...
    {
        //初始化随机数种子
        for ( uint64_t i = 0; i < g_max_thread_num; i++)
        {
        ...
        // UtilFunc::denom = UtilFunc::Zeta(g_ycsb_record_num_per_shard - 1, g_ycsb_zipf_theta);
        // UtilFunc::zeta_2_theta = UtilFunc::Zeta(2, g_ycsb_zipf_theta);
        // UtilFunc::eta = (1 - pow(2.0 / g_ycsb_record_num_per_shard, 1 - g_ycsb_zipf_theta))/
	...
	// static uint64_t Zipf(uint64_t n, double theta)
    // {
    //     double alpha  = 1.0 / (1.0 - theta);
    //     double zetan  = UtilFunc::denom;
    //     double u      = double(UtilFunc::Rand(1000, 0)/1000.0);
    //     double uz     = u * zetan;
    //     if(uz < 1) return 1;
