...
#elif STORAGE_STRATEGY_TYPE == SHARD_BY_WAREHOUSE_SS
...
void TPCCSchema::CreateIndex(IndexID index_id, ShardID shard_id)
{
    ...
    case W_PK_INDEX:
        {
            ...
            indexes_[shard_id * index_cnt_per_shard_ + index_id] = new HashIndex(w_fore_num_per_ware / HASH_FILL_FACTOR, WAREHOUSE_T, true);
        }
        break;
    case D_PK_INDEX:
        {
            uint64_t d_fore_num_per_ware = pow(2, D_W_ID_OFFSET);
            indexes_[shard_id * index_cnt_per_shard_ + index_id] = new HashIndex(d_fore_num_per_ware / HASH_FILL_FACTOR, DISTRICT_T, true);
        }
        break;
    case C_PK_INDEX:
        {
            uint64_t c_fore_num_per_ware = pow(2, C_W_ID_OFFSET);
            indexes_[shard_id * index_cnt_per_shard_ + index_id] = new HashIndex(c_fore_num_per_ware / HASH_FILL_FACTOR, CUSTOMER_T, true);
        }
        break;
    case H_PK_INDEX:
        {
            uint64_t h_fore_num_per_ware = pow(2, H_ID_BITS);
            indexes_[shard_id * index_cnt_per_shard_ + index_id] = new HashIndex(h_fore_num_per_ware / HASH_FILL_FACTOR, HISTORY_T, true);
        }
        break;
    case NO_PK_INDEX:
        {
            uint64_t no_fore_num_per_ware = pow(2, NO_W_ID_OFFSET);
            indexes_[shard_id * index_cnt_per_shard_ + index_id] = new HashIndex(no_fore_num_per_ware / HASH_FILL_FACTOR, NEW_ORDER_T, true);
        }
        break;
    case O_PK_INDEX:
        {
            uint64_t o_fore_num_per_ware = pow(2, O_W_ID_OFFSET);
            indexes_[shard_id * index_cnt_per_shard_ + index_id] = new HashIndex(o_fore_num_per_ware / HASH_FILL_FACTOR, ORDER_T, true);
        }
        break;
    case OL_PK_INDEX:
        {
            uint64_t ol_fore_num_per_ware = pow(2, OL_W_ID_OFFSET);
            indexes_[shard_id * index_cnt_per_shard_ + index_id] = new HashIndex(ol_fore_num_per_ware / HASH_FILL_FACTOR, ORDER_LINE_T, true);
        }
        break;
    case S_PK_INDEX:
        {
            uint64_t s_fore_num_per_ware = pow(2, S_W_ID_OFFSET);
            indexes_[shard_id * index_cnt_per_shard_ + index_id] = new HashIndex(s_fore_num_per_ware / HASH_FILL_FACTOR, STOCK_T, true);
        }
        break;
    case I_PK_INDEX:
        {
            if (shard_id == 0)
            {
                ...
                indexes_[shard_id * index_cnt_per_shard_ + index_id] = new HashIndex(i_fore_num / HASH_FILL_FACTOR, ITEM_T, true); 
            }
            else
            {
                indexes_[shard_id * index_cnt_per_shard_ + index_id] = indexes_[0 * index_cnt_per_shard_ + index_id];
            }
        }
        break;
    case C_LAST_INDEX:
        {
            uint64_t c_fore_num_per_ware = pow(2, C_W_ID_OFFSET);
            indexes_[shard_id * index_cnt_per_shard_ + index_id] = new HashIndex(c_fore_num_per_ware / HASH_FILL_FACTOR, CUSTOMER_T, false);
        }
        break;
...
Index* TPCCSchema::GetIndex(IndexID index_id, ShardID shard_id = 0)
{
