...
constexpr uint64_t NEWORDER_RATIO  = 476;
constexpr uint64_t PAYMENT_RATIO   = 952;
constexpr uint64_t DELIVERY_RATIO  = 1000;


/** TPCC Load & Populate 参数 **/
...
constexpr uint64_t g_max_ol_num_per_order = 15;
constexpr uint64_t g_min_ol_num_per_order = 5;
...
/* 
 * 控制TPCC分布式事务的比例
 * g_new_order_remote_ratio表示每个order_line访问远端仓库的百分比
 * 注意，取1时大约存在10%的neworder事务是分布式事务，取2时大约20%。
 * 
 * g_payment_remote_ratio表示远端customer的百分比
 */
constexpr uint64_t g_new_order_remote_ratio = 1;
constexpr uint64_t g_payment_remote_ratio   = 10;
