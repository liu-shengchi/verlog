...
YCSBSchema::YCSBSchema()
{
    ...
    uint64_t ycsb_forcast_num = pow(2, YT_ALL_BITS);
    indexes_[0] = new HashIndex(ycsb_forcast_num / HASH_FILL_FACTOR, YCSB_TABLE, true);

#elif STORAGE_STRATEGY_TYPE == SHARD_BY_YCSB_SHARD_SS
    shard_cnt_           = g_ycsb_shard_num;
    index_cnt_per_shard_ = g_ycsb_index_num;
    ...
    for (uint32_t i = 0; i < index_cnt_; i++)
    {
        ...
        uint64_t ycsb_forcast_num = pow(2, YT_ALL_BITS);
        indexes_[i] = new HashIndex(ycsb_forcast_num / HASH_FILL_FACTOR, YCSB_TABLE, true);
    }
...
void YCSBSchema::CreateIndex(IndexID index_id, ShardID shard_id)
{
    ...
    {
    case YCSB_INDEX:
        indexes_[shard_id * index_cnt_per_shard_ + index_id] = new HashIndex(g_ycsb_record_num_per_shard, YCSB_TABLE, true);
        break;
    default:
        break;
    }
